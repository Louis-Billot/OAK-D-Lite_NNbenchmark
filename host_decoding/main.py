...
parser.add_argument("-nn", "--nn_model", help="select model path for inference", default='C:/depthai_blob_files/yolov5n_640_fp16_u8_4.blob', type=str)
...
output_shape = int(3 * ((nn_shape/8) ** 2 + (nn_shape/16) ** 2 + (nn_shape/32) ** 2))
...
with dai.Device(pipeline) as device:
    ...
    for i in range(1000):
        ...
        while time.time() < stop_time + period:
            in_nn_input = q_nn_input.get()
            in_nn = q_nn.get()
            sysInfo = qSysInfo.tryGet()
            if sysInfo is not None:
                avg_temp = sysInfo.chipTemperature.average
        ...
        print(f"{frame_counter} frames in {period} seconds so {frame_counter / period:.2f} fps in avg - Max temp: {max_temp:.2f}")
